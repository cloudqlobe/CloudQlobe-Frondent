...
const LeadsMessage = () => {
  ...
  const fetchMessages = async () => {
    try {
      const res = await axiosInstance.get("api/member/getAccountsMessage");
      ...
      allMessages.forEach((msg) => {
        const contactRole = msg.chat_from === "Accounts" ? msg.chat_to : msg.chat_from;
        ...
        if (!msg.read_status && msg.chat_to === "Accounts") {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="bg-white shadow-2xl p-4 lg:col-span-1">
              ...
              <ul>
                {contacts.length > 0 ? (
                  contacts
                  .filter((contacts) => contacts.role !== "Accounts")
                  .map((contact) => (
                    <li
                      key={contact.id}
                    ...
                    {filteredMessages.map((msg, index) => (
                      <div key={index} className={`mb-2 ${msg.sender_id === adminDetails.id ? "text-right" : ""}`}>
                        <div className={`p-3 rounded-2xl shadow-md w-3/4 ${msg.sender_id === adminDetails.id ? "ml-auto bg-green-100" : "bg-blue-100"}`}>
                          ...
                          {!msg.reply_of_message && msg.chat_to === "Accounts" && (<FaReply className="text-gray-600 cursor-pointer mt-1" onClick={() => openReplyModal(msg)} />)}
                          {msg.reply_of_message && msg.chat_from === "Accounts" && (
