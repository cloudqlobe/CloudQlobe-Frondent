...
const CarriersPage = () => {
  ...
  const handleCarrierClick = (id) => {
    window.location.href = `/modules/admin/Carriers/${id}`;
  };
      ...
      <div className="mb-4">
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg ${
            activeTab === 'followup' ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-700'
          }`}
        >
          Follow Up
        </button>
                ...
                {sortedCarriers.map((carrier) => (
                  ...
                  >
                    <td className="py-2 px-4">
                      {carrier.userFirstname} {carrier.userLastname}
                    </td>
                    <td className="py-2 px-4">{carrier.companyName}</td>
                    <td
                      className={`py-2 px-4 ${
                        carrier.customerStatus === 'active' ? 'text-green-600' : 'text-red-600'
                      }`}
                    >
