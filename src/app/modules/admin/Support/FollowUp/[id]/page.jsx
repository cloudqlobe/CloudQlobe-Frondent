...
const FollowUpDetails = () => {
  ...
  // Fetch follow-up details based on the ID (using hardcoded ID for demonstration)
  useEffect(() => {
    const fetchFollowUp = async () => {
      try {
        const response = await axiosInstance.get('v3/api/followups/1'); // Use a static ID for demo
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      ...
      await axiosInstance.put(`v3/api/followups/1`, updatedFollowUpData);
